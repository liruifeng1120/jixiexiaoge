...
def _create_radar_can_parser(car_fingerprint):
  messages: list[tuple[str | int, int]] = [(f"RADAR_TRACK_{addr:x}", 10) for addr in RADAR_TRACK_ADDRS]
...
class RadarInterface(RadarInterfaceBase):
  ...
  def _update(self, updated_messages):
    ...
    for addr in RADAR_TRACK_ADDRS:
      msg = rcp.vl[f"RADAR_TRACK_{addr:x}"]
      ...
      if dist == SENTINEL_DIST or ang == SENTINEL_ANG or relv == SENTINEL_RELV or addr not in RADAR_USABLE_ADDRS:
        if addr in self.pts:
          del self.pts[addr]
        continue
